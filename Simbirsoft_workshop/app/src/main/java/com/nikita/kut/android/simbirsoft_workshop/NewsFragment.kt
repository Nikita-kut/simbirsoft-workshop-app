        ...
        try {
            ...
            for (i in 0 until newsArray.length()) {
                ...
                for (index in 0 until categoriesArray.length()) {
                    ...
                    if (children.length > 1) enumCategory.add(CategoriesOfHelp.valueOf(children))
                    val adult = currentCategory.getString("adult")
                    if (adult.length > 1) enumCategory.add(CategoriesOfHelp.valueOf(adult))
                    val elderly = currentCategory.getString("elderly")
                    if (elderly.length > 1) enumCategory.add(CategoriesOfHelp.valueOf(elderly))
                    val animals = currentCategory.getString("animals")
                    if (animals.length > 1) enumCategory.add(CategoriesOfHelp.valueOf(animals))
                    val events = currentCategory.getString("events")
                    if (events.length > 1) enumCategory.add(CategoriesOfHelp.valueOf(events))
                }
